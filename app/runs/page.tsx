    ...
    <div className="px-4 pt-6 pb-4 md:px-8 md:pt-10 max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        ...
        <button onClick={() => setShowAdd(!showAdd)}
          className="flex items-center gap-2 px-3 py-2 rounded-xl bg-[#0f0f0f] text-[#555] hover:text-[#888] hover:bg-[#151515] border border-[#1a1a1a] text-sm transition-colors flex-shrink-0">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-8">
        {[
          ...
          <div key={label} className="rounded-xl border border-[#1e1e1e] bg-[#0c0c0c] p-4">
            <div className="flex items-center justify-between mb-2">
              <span className="text-[10px] tracking-[0.2em] text-[#444] uppercase">{label}</span>
      ...
      {showAdd && (
        <div className="rounded-xl border border-[#222] bg-[#0c0c0c] p-6 mb-6">
          <h3 className="text-sm font-medium text-[#888] mb-4 tracking-wide">Log a Run</h3>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-3">
            ...
            <div><label className="text-[11px] text-[#444] block mb-1">Name (optional)</label><input placeholder="Morning run" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} /></div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3 mb-4">
            <div><label className="text-[11px] text-[#444] block mb-1">Miles *</label><input type="number" step="0.01" placeholder="6.2" value={form.distanceMiles} onChange={e => setForm({ ...form, distanceMiles: e.target.value })} /></div>
            <div><label className="text-[11px] text-[#444] block mb-1">Duration (min) *</label><input type="number" placeholder="50" value={form.durationMin} onChange={e => setForm({ ...form, durationMin: e.target.value })} /></div>
            <div><label className="text-[11px] text-[#444] block mb-1">Avg HR</label><input type="number" placeholder="155" value={form.avgHr} onChange={e => setForm({ ...form, avgHr: e.target.value })} /></div>
            <div><label className="text-[11px] text-[#444] block mb-1">Elevation (ft)</label><input type="number" placeholder="420" value={form.elevationFt} onChange={e => setForm({ ...form, elevationFt: e.target.value })} /></div>
            <div><label className="text-[11px] text-[#444] block mb-1">Calories</label><input type="number" placeholder="600" value={form.calories} onChange={e => setForm({ ...form, calories: e.target.value })} /></div>
          </div>
      ...
      <div className="space-y-2">
        {runs.length === 0 && <p className="text-[#333] text-sm py-8 text-center">No runs yet. Sync your Coros or log manually.</p>}
        {runs.map(run => (
          <div key={run.id} className="rounded-xl border border-[#1a1a1a] bg-[#0c0c0c] hover:border-[#252525] transition-colors p-5">
            <div className="flex items-center gap-3 mb-3">
              <div className="w-2 h-2 rounded-full flex-shrink-0" style={{ backgroundColor: TYPE_COLORS[run.type] ?? "#4a7c59" }} />
              <span className="text-[11px] tracking-[0.2em] text-[#444] uppercase">{run.type}</span>
              <span className="text-[11px] text-[#333] ml-auto">{new Date(run.date).toLocaleDateString("en-US", { weekday: "short", month: "short", day: "numeric", year: "numeric" })}</span>
            </div>
            <div className="flex items-baseline gap-6">
              <div>
                <span className="text-2xl font-bold text-white">{run.distanceMiles.toFixed(2)}</span>
                <span className="text-sm text-[#444] ml-1">mi</span>
              </div>
              {run.avgPaceSecMile && (
                <div>
                  <span className="text-lg font-semibold text-[#888]">{formatPaceMile(run.avgPaceSecMile)}</span>
                  <span className="text-xs text-[#444] ml-1">/mi</span>
                </div>
              )}
              <span className="text-[#444] text-sm">{formatDuration(run.durationSeconds)}</span>
            </div>
...
function LoadingScreen() {
  return <div className="flex items-center justify-center h-screen"><div className="text-[#333] text-sm tracking-widest uppercase animate-pulse">Loading</div></div>;
}

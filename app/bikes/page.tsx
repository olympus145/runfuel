  ...
  return (
    <div className="px-4 pt-8 pb-6 md:px-8 md:pt-12 max-w-4xl mx-auto">
      ...
      <div className="grid grid-cols-3 gap-3 mb-8">
        <div className="rounded-xl border border-[#1a1a1a] bg-[#0c0c0c] p-5">
          <div className="flex items-center justify-between mb-3"><span className="text-[10px] tracking-[0.2em] text-[#666] uppercase">Miles</span><Bike size={13} color="#3b82f6" strokeWidth={1.5} /></div>
          <p className="text-xl font-bold text-white">{totalMiles.toFixed(1)}</p>
          <p className="text-[10px] text-[#666] mt-1">this week</p>
        </div>
        <div className="rounded-xl border border-[#1a1a1a] bg-[#0c0c0c] p-5">
          <div className="flex items-center justify-between mb-3"><span className="text-[10px] tracking-[0.2em] text-[#666] uppercase">Time</span><Zap size={13} color="#c8a96e" strokeWidth={1.5} /></div>
          <p className="text-xl font-bold text-white">{formatDuration(totalTime)}</p>
          <p className="text-[10px] text-[#666] mt-1">this week</p>
        </div>
        <div className="rounded-xl border border-[#1a1a1a] bg-[#0c0c0c] p-5">
          <div className="flex items-center justify-between mb-3"><span className="text-[10px] tracking-[0.2em] text-[#666] uppercase">Rides</span><Bike size={13} color="#3b82f6" strokeWidth={1.5} /></div>
      ...
      {showAdd && (
        <div className="rounded-xl border border-[#252525] bg-[#0c0c0c] p-6 mb-6">
          <h3 className="text-sm font-medium text-[#aaa] mb-5">Log a Ride</h3>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-3">
            ...
            <div><label className="Field-label">Name</label><input placeholder="Morning ride" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} /></div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
            <div><label className="Field-label">Miles *</label><input type="number" step="0.01" placeholder="25.0" value={form.distanceMiles} onChange={e => setForm({ ...form, distanceMiles: e.target.value })} /></div>
            <div><label className="Field-label">Duration (min) *</label><input type="number" placeholder="75" value={form.durationMin} onChange={e => setForm({ ...form, durationMin: e.target.value })} /></div>
            <div><label className="Field-label">Avg HR</label><input type="number" placeholder="145" value={form.avgHr} onChange={e => setForm({ ...form, avgHr: e.target.value })} /></div>
            <div><label className="Field-label">Elevation (ft)</label><input type="number" placeholder="1200" value={form.elevationFt} onChange={e => setForm({ ...form, elevationFt: e.target.value })} /></div>
          </div>
      ...
      <div className="space-y-2">
        {rides.length === 0 && <p className="text-[#666] text-sm py-10 text-center">No rides yet. Sync your Coros or log manually.</p>}
        {rides.map(ride => (
          <div key={ride.id} className="rounded-xl border border-[#1a1a1a] bg-[#0c0c0c] hover:border-[#282828] p-5 transition-colors">
            <div className="flex items-center gap-3 mb-3">
              <div className="w-2 h-2 rounded-full bg-[#3b82f6] flex-shrink-0" />
              <span className="text-[11px] tracking-[0.2em] text-[#777] uppercase">{ride.type.replace("_", " ")}</span>
              <span className="text-[11px] text-[#666] ml-auto">{new Date(ride.date).toLocaleDateString("en-US", { weekday: "short", month: "short", day: "numeric", year: "numeric" })}</span>
            </div>
            {ride.name && <p className="text-sm text-[#aaa] mb-2">{ride.name}</p>}
            <div className="flex items-baseline gap-6">
              <div><span className="text-2xl font-bold text-white">{ride.distanceMiles.toFixed(2)}</span><span className="text-sm text-[#777] ml-1">mi</span></div>
              {ride.avgSpeedMph && <div><span className="text-lg font-semibold text-[#bbb]">{ride.avgSpeedMph.toFixed(1)}</span><span className="text-xs text-[#777] ml-1">mph avg</span></div>}
              <span className="text-[#888] text-sm">{formatDuration(ride.durationSeconds)}</span>
            </div>
